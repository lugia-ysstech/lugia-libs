...
export function getFontSize(domNode: any, defaultFontSize: number): number {
  if (!domNode) {
...
export function getSizeByStyle(style: SizeStyle): SizeType {
  if (!style) {
...
export function size2Style(size: SizeType): SizeStyle {
  if (!size) {
    return {};
  }
...
export function getPointByStyle(style: PointStyle): Point {
  if (!style) {
    return [0, 0];
  }
...
export function point2Style(point: Point): PointStyle {
  if (!point) {
    return {};
  }
  const [x, y] = point;
